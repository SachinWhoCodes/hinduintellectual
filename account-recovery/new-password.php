...
<head>
    ...
    <title>Set New Password - Hindu Intellectual</title>
...
<div class="login_box">
    <div class="login_space">
        <div class="login_heading">
            <h1 class="laila-bold" >New Password</h1>
        ...
        if (!empty($phone) && !empty($entered_email)) {
            ...
            if ($result->num_rows > 0) {
                ...
                if ($entered_email === $stored_email) {
                    // updating password from here
                    
                    $conn_update = new mysqli($host, $username, $password, $database);
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                        ...
                        if (!empty($new_password) && !empty($entered_email) && !empty($phone)) {
                            // Sanitize inputs
                            $new_password = $conn_update->real_escape_string($new_password);
                            $entered_email = $conn_update->real_escape_string($entered_email);
                            $phone = $conn_update->real_escape_string($phone);
                            $hashed_password = password_hash($new_password, PASSWORD_BCRYPT);
                    
                            // Update the password where email and phone match
                            $query = "UPDATE users SET password = '$hashed_password' WHERE email = '$entered_email' AND phone = '$phone'";
                            if ($conn_update->query($query) === TRUE) {
                                echo "<p>Password updated successfully.</p>";
                                $_SESSION['recoStat'] = 3;
                                echo '<script>window.location.href = "https://hinduintellectual.com/login.php";</script>';
                            } else {
                                echo "<p>Error updating password: " . $conn_update->error . "</p>";
                            }
                        } else {
                            echo "<p>Please ensure all fields are filled in correctly.</p>";
                        }
                    }
...
?>
        ...
        <form class="main_login_form" action="" method="post" >
            <div class="input_box">
                ...
                <input name="password" class="laila-regular" type="text" placeholder="Set New Strong Password">
            </div>
            <p class="laila-regular terms_noti">By loggin in, you are agree with our <a href="#">T&C</a> and <a href="#">Privacy Policy</a></p>
            <button class="laila-regular" type="submit">Submit</button>
        </form>
        <?php
